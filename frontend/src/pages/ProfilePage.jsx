...
import { 
  ...
  ChevronLeft, Save, Edit2, ShieldCheck, Loader2,
  BookOpen, Hash, Award 
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#FDFCFB] p-6 md:p-12 font-sans text-slate-900">
      ...
      <div className="max-w-5xl mx-auto mb-8 flex items-center justify-between">
        ...
        <h1 className="text-2xl font-serif font-bold text-slate-800">My Profile</h1>
      </div>

      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="max-w-5xl mx-auto bg-white rounded-[2.5rem] border border-gray-100 shadow-2xl overflow-hidden"
      >
        <div className="h-40 bg-gradient-to-r from-teal-700 to-slate-800" />

        <div className="px-10 pb-12">
          {/* Action Bar */}
          <div className="relative -mt-20 mb-12 flex flex-col md:flex-row justify-between items-end gap-6">
            ...
            <div className="flex-1">
                <h2 className="text-3xl font-serif font-bold text-slate-900">{formData.fullName}</h2>
                <p className="text-teal-600 font-medium">{formData.course} • {formData.studentId}</p>
            </div>

            <button
              onClick={() => isEditing ? handleUpdate() : setIsEditing(true)}
              className={`flex items-center gap-2 px-8 py-3 rounded-2xl font-bold transition-all ${
                isEditing ? 'bg-orange-600 text-white' : 'bg-slate-900 text-white'
              }`}
            >
